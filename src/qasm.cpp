                         ...
                         const std::map< std::string, OPCodeInfo >& opcode_map )
{
    ...
    while( token_idx < tokens.size() )
    {
        ...
        if( token.cat == Token::OPCODE )
        {
            ...
            if( it == opcode_map.end() )
                throw std::runtime_error( "Unknown opcode: " + token.data + " at line " +
            ...
            if( token_idx >= tokens.size() || ( tokens[ token_idx ].cat != Token::NUMBER &&
                ...
                throw std::runtime_error( "Missing operand for " + token.data + " at line " +
                                          std::to_string( token.loc.line ) + ", col " +
                                          std::to_string( token.loc.col ) );
            ...
            if( info.format == "i32" || info.format == "const" )
            {
                if( operand.cat == Token::NUMBER )
                {
                    ...
                    // rewrite this using the new api, e.g emit_add( int32 num )
            ...
            else if( info.format == "i16" || info.format == "u16" || info.format == "loc" ||
                     info.format == "arg" || info.format == "var_ref" ||
                     info.format == "npop_u16" || info.format == "npop" )
            {
                if( operand.cat == Token::NUMBER )
                {
            ...
            else if( info.format == "i8" || info.format == "u8" || info.format == "loc8" ||
                     info.format == "const8" )
            {
                if( operand.cat == Token::NUMBER )
                {
    ...
    // trick: trieda ktora ma append a inkrementuje pc
...
bytes assemble_file( const std::string& content )
{
    ...
    while( token_idx < tokens.size() )
    {
        if( tokens[ token_idx ].cat == Token::END )
            break;
