...
decode_utf8(std::string_view f, bool report_partial = true) {
    ...
    if (a < 0xF0) {
        ...
        switch (a) {
            case 0xE0: if ((b & 0xe0) != 0xa0) return error;
                else break;
            case 0xED: if ((b & 0xe0) != 0x80) return error;
                else break;
            default: if ((b & 0xc0) != 0x80) return error;
                else break;
        }
    ...
    if (a < 0xF5) {
        ...
        switch (a) {
            case 0xF0: if (b < 0x90 || b > 0xBF) return error;
                else break;
            case 0xF4: if ((b & 0xf0) != 0x80) return error;
                else break;
            default: if ((b & 0xc0) != 0x80) return error;
                else break;
        }
